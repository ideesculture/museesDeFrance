...
class ca_data_exporters extends BundlableLabelableBaseModelWithAttributes {
	...
	public function getFileExtension() {
		...
		switch($this->getSetting('exporter_format')) {
			...
			case 'CSV':
				...
				break;
			case 'LABELEDTXT':
				$o_export = new ExportLabeledText();
				break;
			case 'ExifTool':
	...
	static public function exportRecordsFromSearchResult($ps_exporter_code, $po_result, $ps_filename, $pa_options=array()) {
		...
		if ($t_mapping->getSetting('CSV_print_field_names')) {
			...
			foreach($va_mapping_items as $vn_i => $va_mapping_item) {
				...
				//var_dump($va_mapping_item['element'] );
			}
	...
	public function processExporterItem($pn_item_id,$pn_table_num,$pn_record_id,$pa_options=array()) {
		...
		if($vs_leading_zeros_for_length =  $t_exporter_item->getSetting('leading_zeros_for_length')) {
			$va_get_options['dateFormat'] = $vs_leading_zeros_for_length;
		}
		...
		$vs_leading_zeros_for_length =  $t_exporter_item->getSetting('leading_zeros_for_length');

		//echo("\nhere\n");
		//var_dump($vs_prefix);
		//var_dump($vs_leading_zeros_for_length);
		//die();
		...
		foreach($va_item_info as $vn_key => &$va_item) {
			...
			if(strlen($va_item['text'])==0) {
				...
				if((int)$vs_leading_zeros_for_length>0) {
					$va_item['text'] = sprintf("%'0".(int)$vs_leading_zeros_for_length."s", $va_item['text']);
				}
